...
test("generates byte-stable accessories and leaves every TASK-009 file unchanged", async () => {
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "production-lite-head-accessory-generation-"),
  );
  const snapshot = await createReadOnlyExamplesSnapshot(
    repositoryRoot,
    temporaryRoot,
    ["production-lite-character", "production-lite-head-accessories"],
  );
  ...
  try {
    await execFileAsync(
      ...
      [
        path.join(
          packageRoot,
          "scripts/generate-production-lite-head-accessories.mjs",
        ),
        ...rootArguments,
      ],
      { cwd: packageRoot },
    ...
    await execFileAsync(
      ...
      [
        path.join(
          packageRoot,
          "scripts/verify-production-lite-head-accessories.mjs",
        ),
        ...rootArguments,
      ],
      { cwd: packageRoot },
    ...
    assert.deepEqual(
      await digests(generatedFixtureRoot, generatedFiles),
      before,
    );
    assert.deepEqual(
      await digests(generatedCocosRoot, generatedFiles),
      mirrorBefore,
    );
