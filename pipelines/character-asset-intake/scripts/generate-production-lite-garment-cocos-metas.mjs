...
const source = JSON.parse(
  await readFile(
    path.join(
      repositoryRoot,
      "examples/production-lite-garment-layering/source/garment-source.json",
    ),
    "utf8",
  ),
);
...
const uuid = (key) => {
  const hex = createHash("sha256").update(`task-011:${key}`).digest("hex").slice(0, 32);
...
await write(rootMeta, directoryMeta("root"));
for (const folder of ["attachments", "animations", "reference"]) {
  await write(path.join(root, `${folder}.meta`), directoryMeta(folder));
}
await write(
  path.join(root, "attachment-layout.json.meta"),
  jsonMeta("attachment-layout.json"),
);
await write(
  path.join(root, "animations/garment-stress.json.meta"),
  jsonMeta("animations/garment-stress.json"),
);
await write(
  path.join(root, "reference/authoring-provenance.json.meta"),
  jsonMeta("reference/authoring-provenance.json"),
);
