...
const sceneFile = path.join(
  assetsRoot,
  "task-013r3-single-attachment-bridge.scene",
);
const sceneMetaFile = `${sceneFile}.meta`;
const scriptFile = path.join(
  assetsRoot,
  "gameai/task013r3/task013r3-single-attachment-bridge.ts",
);
...
test("TASK-013R3 Creator-owned scene resolves exactly one single-attachment component", async () => {
...
test("TASK-013R3 scene is newly Creator-owned and contains only Canvas plus its Camera", async () => {
  ...
  assert.doesNotMatch(
    sceneText,
    /composable-full-loadout|head-accessory-layering|task-013r2-base/iu,
  );
});

test("TASK-013R3 every runtime module has imported Creator-owned metadata", async () => {
  for (const moduleName of [
    "base-rig-runtime-builder.ts",
    "single-attachment-input-registry.ts",
    "single-attachment-plan-data.ts",
    ...
    "task013r3-single-attachment-bridge.ts",
  ]) {
    const meta = await json(
      path.join(assetsRoot, "gameai/task013r3", `${moduleName}.meta`),
    );
...
test("TASK-013R3 scene participates in the tracked-files-only metadata audit", async () => {
  const result = await validateTrackedCocosScenes(assetsRoot);
  assert.ok(result.sceneCount >= 15);
  assert.ok(result.customComponentCount >= 14);
  assert.ok(result.scriptClassCount >= 35);
});
