...
const sceneFile = path.join(
  assetsRoot,
  "task-013r4-head-accessory-layering.scene",
);
const sceneMetaFile = `${sceneFile}.meta`;
const scriptFile = path.join(
  assetsRoot,
  "gameai/task013r4/task013r4-head-accessory-layering-bridge.ts",
);
...
test("TASK-013R4 Creator-owned scene resolves exactly one multi-attachment component", async () => {
...
test("TASK-013R4 scene is newly Creator-owned and contains only Canvas plus its Camera", async () => {
  ...
  assert.doesNotMatch(
    sceneText,
    /composable-full-loadout|task-013r3-single|task-013r2-base/iu,
  );
});

test("TASK-013R4 every runtime module has imported Creator-owned metadata", async () => {
  for (const moduleName of [
    "multi-attachment-input-registry.ts",
    "multi-attachment-plan-data.ts",
    ...
    const meta = await json(
      path.join(assetsRoot, "gameai/task013r4", `${moduleName}.meta`),
    );
...
test("TASK-013R4 scene participates in the tracked-files-only metadata audit", async () => {
  const result = await validateTrackedCocosScenes(assetsRoot);
  assert.ok(result.sceneCount >= 16);
  assert.ok(result.customComponentCount >= 15);
  assert.ok(result.scriptClassCount >= 43);
});
