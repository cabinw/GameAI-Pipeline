...
const componentUuid = "98035dbf-6477-43bb-b88c-af712f4fa1a9";
const fallbackSceneUuid = "2862763d-3bee-483b-a34e-9a0964acc124";
let sceneUuid = fallbackSceneUuid;
try {
  const metadata = JSON.parse(await readFile(`${outputFile}.meta`, "utf8"));
  if (typeof metadata.uuid !== "string") {
    throw new Error("TASK_014D2_SCENE_META_UUID_INVALID");
  }
  sceneUuid = metadata.uuid;
} catch (error) {
...
for (const entry of document) {
  if (entry?._name === "task-014c-canonical-loadout-semantic-vfx") {
    entry._name = "task-014d2-cocos-vfx-render-plan-adapter";
  }
  if (entry?.__type__ === sourceType) {
    entry.__type__ = componentType;
  }
}
...
if (
  ...
  throw new Error("TASK_014D2_SCENE_TEMPLATE_INVALID");
}
scene._id = sceneUuid;
